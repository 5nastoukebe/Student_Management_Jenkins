        ...
        stage('Deploye Code') {
            if (isUnix()) {
                rc = sh returnStatus: true, script: """
                     ${sfdxPath} force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile ${jwt_key_file} --setdefaultdevhubusername --instanceurl ${SFDC_HOST}
                """
            }else{
